-module(node).
-export([start/1,start/2,stop/0,abroadcast/1,adeliver/2]).
...
start(N,ADeliver)->case net_adm:ping(N) of %%N nodo de la red o yo mismo si soy el primero
                       pong->register(sender,spawn(?MODULE,senderinit,[ADeliver]));
                       pang->red_no_alcanzada
                   end.

%%start con handler por defecto(imprimir mensaje)
start(N)->start(N,fun(X)->io:format("~p~n",[X])end).
...
adeliver([{M,_,_,_,deliverable}|Queue],ADeliver)->ADeliver(M),adeliver(Queue,ADeliver);
...
senderinit(ADeliver)->
    ...
    senderloop(0,0,[],maps:new(),ADeliver).

%%loop principal
senderloop(S,Counter,Queue,Responses,ADeliver)->
    receive
        ...
        {broadcast,M}->?DEBUG("broadcast(~p)~n",[M]),
                       timer:sleep(?SLEEP),
                       lists:foreach(fun(X)->{sender,X}!{sender_propose,M,Counter,node()}end,nodes()),
                       senderloop(S,Counter+1,Queue,maps:put(Counter,spawn(?MODULE,votebox,[S,Counter,nodes()]),Responses),ADeliver);
        ...
        {sender_propose,M,Id,From}->?DEBUG("sender_propose(~p,~p,~p)~n",[M,Id,From]),
                                    ...
                                    senderloop(S+1,Counter,[{M,Id,From,S,undeliverable}|Queue],Responses,ADeliver);
        ...
        {agreed,Id,From,SMax}->?DEBUG("agreed(~p,~p,~p)~n",[Id,From,SMax]),
                               ...
                               senderloop(max(S,SMax),Counter,adeliver(S2Queue,ADeliver),Responses,ADeliver);
        ...
        {voting_finished,Id,SMax}->?DEBUG("voting_finished(~p,~p)~n",[Id,SMax]),
                                   timer:sleep(?SLEEP),
                                   lists:foreach(fun(X)->{sender,X}!{agreed,Id,node(),SMax}end,nodes()),
                                   senderloop(SMax,Counter,Queue,maps:remove(Id,Responses),ADeliver);
...
votebox(S,Id,Responses)->
    receive {check_vote,Id,SO,From}->?DEBUG("check_vote(~p,~p,~p)~n",[Id,SO,From]),
