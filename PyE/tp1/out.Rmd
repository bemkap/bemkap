...
* **Recorridos**: datos sobre los viajes
  ...
  + Variables:
    + id_usuario: identificador correspondiente a la base Usuarios
...
barplot(sort(GTG,decreasing=T),col="lightblue",ylim=c(0,60),ylab="Porcentaje",cex.axis=0.8,cex.names=0.8,width=10,xlim=c(0,50),space=0.5)
box()
mtext("Gráfica 1: Porcentaje de usuarios por género")
```
...
```{r, echo=FALSE, fig.align='center'}
boxplot(usuarios$edad_usuario,horizontal=T,xlab="Edad",ylim=c(0,80),col="lightblue")
mtext("Gráfica 2: Cantidad de usuarios por edad")
```
...
mtext("Gráfica 3: Frecuencia de estaciones según cantidad de registros")
```
...
```{r, echo=FALSE, fig.align='center'}
par(cex=0.75)
pie(TEO[,2],init.angle=-45,labels=paste(TEO[,2]*100,"%",str_trunc(dimnames(TEO)[[1]],20)))
box()
mtext("Gráfica 4: Porcentajes de las 10 estaciones de origen con más recorridos",cex=1.1)
```
Las 10 estaciónes de origen más frecuentes representan aproximadamente el 25% de la muestra, es decir, el primer cuartil. De un total de 380 registros, se redujo a 98. Y la más común es "TTE. GENERAL JUAN DOMINGO PERON".
...
mtext("Gráfica 5: Frecuencia de estaciones según cantidad de registros")
```
...
box()
mtext("Gráfica 6: Porcentaje de las 10 estaciones de destino con más recorridos",cex=1.1)
```
...
```{r, echo=FALSE, fig.align='center'}
boxplot(recorridos$duracion_recorrido/3600,horizontal=T,ylim=c(0,5),range=9,col="lightblue",xlab="Tiempo (en horas)")
mtext("Gráfica 7: Cantidad de recorridos según su duración",cex=1.1)
```
...
Medidas descriptivas
```{r, echo=FALSE, fig.align='center'}
hist(recorridos$distancia,ylim=c(0,150),xlab="Distancia (en metros)",ylab="Cantidad",col="lightblue",main="")
box()
mtext("Gráfica 8: Cantidad de recorridos según su distancia",cex=1.1)
```
...
Medidas descriptivas
```{r, echo=FALSE, fig.align='center'}
barplot(sort(table(recorridos$dia),decreasing=T),horiz=T,las=1,xlim=c(0,70),col="lightblue",cex.names=0.8,xlab="Cantidad")
box()
mtext("Gráfica 9: Cantidad de recorridos según el día",cex=1.1)
```
...
```{r, echo=FALSE, fig.align='center'}
barplot(GGET,ylim=c(0,20),col=c("lightblue","green","orange"),ylab="Cantidad",xlab="Edad",beside=T,legend.text=c("Femenino","Masculino","Otro"))
box()
mtext("Gráfica 10: Cantidad de personas por género y edad",cex=1.1)
```
