...
upd_cal=: 3 : 0
 ...
 set_table_data 'cal';boxtoitem 42{.'%6s\n%6d'&(<@:sprintf)"1 C,.&,((<0);<a:) stringreplace T
 tops=. jd SUMTOP sprintf AA,MM
 set_table_data 'tops';boxtoitem,|:8 9$72{.{.<"1&>(,.6&":)&.>/"1 }.tops
)
...
upd_ahorro=: 3 : 0
 upd_total >,.&.>/<"_1&.>{:"1 jd'read from AHOR'
)
...
upd_total=: 3 : 0
 ...
 wd'set total text ','%d USD  ~  %d ARS' sprintf <.(,(%&D))(;1&{"1 y)(+/ .*)&x:(1,D,D){~(;:'ARS USD USC')i.{:"1 y
)

main_resize=: 3 : 'grph_paint gsum_paint upd_ahorro upd_cal upd_meses upd_clientes CL=: /:~~.<"1(0 1){::jd''read cl from VIAJ'''

main_meses_mbldbl=: 3 : 'grph_paint gsum_paint upd_cal ''AA MM''=: 21 0+".meses'

gsum_paint=: 3 : 0
 Q=. jd'read sum pg by dd from VIAJ where aa=%d and mm=%d' sprintf AA,MM
 T=. (W=. weekday(2000+AA),.MM,.0 1{::Q)(+/%#)/.1 1{::Q
 T=. T((~.W)})7$0
 glsel'gsum'
 glclear''
 glpen 2
...
grph_paint=: 3 : 0
 ...
 M=. >./;T=. (1{::Q)</.~6 7 e.~weekday(2000+AA),.MM,.0{::Q
 glsel'grph'
 glclear''
 glpen 2
...
main_cal_mbldbl=: 3 : 'upd_clientes DD=: ".6{.wd''get cal cell '',cal'
